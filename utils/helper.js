...
class Helper{
	...
	async insertMessages(params){
		try {
			...
			return await this.db.query(
				"INSERT INTO messages (`from_user_id`,`to_user_id`,`username`,`mensaje`,`channel`, `channel_type`) values (?,?,?,?,?,?)",
				[(params.fromUserId), (params.toUserId),params.username, this.htmlEscape(params.message),params.channel,params.channel_type]
			);
		} catch (error) {
			console.warn(error);
			return null;
		...
		try {
			return await this.db.query(
				`SELECT * FROM ( SELECT id,from_user_id as fromUserId,to_user_id as toUserId, username, mensaje,fecha FROM messages WHERE 
					(channel = ? ) ORDER BY id DESC LIMIT 100) sub ORDER BY id ASC;`,
				[this.htmlEscape(channel)]
			);
		} catch (error) {
			console.warn(error);
			return null;
	...
	async getMyMessages(userId, toUserId){
		try {
			return await this.db.query(
				`SELECT id,from_user_id as fromUserId,to_user_id as toUserId,mensaje,fecha FROM messages WHERE 
